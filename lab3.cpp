...
{
   /*
   cout << "Vrh: " << vrh.brojVrha << " -> ";
   printVector(vrh.susjedi);
   cout << endl;
   */
...
int main(void)
{
   ...
   ifstream MyFile("C:\\Java\\UltimateEdition\\graf.txt");
   ...
   for (int i = 1; i <= brojVrhova; i++) // ide od jedan da razlika bude tocna
   {
      Vertex v;
      v.brojVrha = i;
      for (int j = 1; j <= brojVrhova; j++) // vrhovi su 1 2 3 ...
      {
         int razlika = abs(i - j);
   ...
   /*
   for (Vertex vrh : vrhoviGrafa)
      vrh.zabranjeneBoje.resize(maxSusjed);
   */
